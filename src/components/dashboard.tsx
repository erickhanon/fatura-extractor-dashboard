...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/faturas')
            .then(response => {
                ...
                const uniqueClientNumbers: string[] = Array.from(new Set(faturas.map((fatura: any) => fatura.client_number)));
    ...
    const filterFaturas = (clientNumber: string) => {
        if (clientNumber && clientNumber !== 'all') {
            setFilteredFaturas(faturas.filter(fatura => fatura.client_number === clientNumber));
        } else {
            setFilteredFaturas(faturas);
        }
    };
    ...
    const calculateMonetaryData = (data: any[]) => {
        return data.map(fatura => ({
            month: fatura.reference_month,
            totalValue: fatura.energia_eletrica_valor + fatura.energia_scee_valor + fatura.contrib_ilum_valor,
            savings: fatura.energia_compensada_valor,
        }));
    };
    ...
    return (
        <Card className="max-w-7xl mx-auto my-8 p-6">
            ...
            <CardContent>
                <div className="mb-8">
                    <h2 className="text-xl font-semibold mb-4">Consumo de Energia (kWh)</h2>
                    <Line data={energyChartData} />
                </div>
                <div>
                    <h2 className="text-xl font-semibold mb-4">Valores Monetários (R$)</h2>
